...
> **注意**: `cdk/` の IAM ロール・S3 バケットポリシー・Lambda のメモリ/タイムアウト等は、利用する AWS アカウントや運用方針に合わせて適宜カスタマイズしてください（例: 複数ユーザで共有するアカウントでは `AccountPrincipal` を特定のロール ARN に絞る、S3 に暗号化・SSL 強制を追加する 等）。
...
upload_file = "./a.out"                 # Lambda で実行するファイル (例: ./target/release/xxx, ./a.out, ./solution.py)
# parallel = 10                         # 同時Lambda起動数 (デフォルト: 10)
...
`phst run --lambda` では次のように処理が分担されます:

- **`compile_steps` はローカルで実行** — `cargo build --release` 等のビルド処理はローカル環境で走ります (Lambda にはツールチェインが無いため)。ビルド成果物はローカルに出力されます。
- **`test_steps` は Lambda 上で実行** — 生成された入力に対する解法実行・スコア算出は Lambda 上で行われます。

そのため、ビルド成果物 (Rust/C++ のバイナリ) やスクリプト本体 (Python 等) の **実体ファイルを Lambda に送り込む必要** があります。これを指定するのが `upload_file` です。

`upload_file` には `test_steps` で実行されるビルド成果物またはスクリプトをワークスペース相対パスで指定してください。ファイルは Lambda 実行時の作業ディレクトリ直下に同名 (basename) で配置されるので、`test_steps` からはそのファイル名で参照できます。Rust/C++ のビルド済みバイナリだけでなく Python 等のスクリプトもサポートされます。

#### Lambda 用に compile_steps / test_steps を上書きする

`[aws_lambda.test]` セクションで `compile_steps` と `test_steps` を個別に上書きできます。指定された側は通常の `[test]` セクションの値を**完全に置き換え**ます (マージではない)。ローカル実行と Lambda 実行でビルドコマンドや実行方法を変えたい場合に使います。

例: ローカルではデバッグビルド、Lambda では `-DLAMBDA` を付けて別ビルドにする場合:

```toml
[test]
# ローカル用 (手元でデバッグしやすいフラグ)
[[test.compile_steps]]
program = "g++"
args = ["-std=c++17", "-O2", "-Wall", "-o", "a.out", "solution.cpp"]
...
tools_bucket = "ahc-tester-tools-XXX"
upload_file = "./a.out"

# Lambda 用: -DLAMBDA を定義して条件コンパイル
[[aws_lambda.test.compile_steps]]
program = "g++"
args = ["-std=c++17", "-O2", "-DLAMBDA", "-o", "a.out", "solution.cpp"]

# test_steps を省略すれば [test.test_steps] がそのまま使われる
```

未指定の場合は通常の `[test]` 以下がそのまま使われるため、上書きが必要なとき (条件付きビルド、Lambda 非対応コマンドの回避等) だけ記述すれば OK です。

#### Python ソリューションを実行する例

ローカルの `[test.test_steps]` が `python3 ./solution.py` 形式なら、`upload_file` を追加するだけで Lambda 実行も動きます (上書き不要):
...
function_name = "ahc-tester"
tools_bucket = "ahc-tester-tools-XXX"
upload_file = "./solution.py"
```

`upload_file` で指定したファイルが Lambda の作業ディレクトリ直下に配置され、`test_steps` の `./solution.py` 参照と一致するので、そのまま実行できます。
...
本リポジトリの CDK 設定では Lambda は **x86_64 (amd64) Linux / Amazon Linux 2023** で動作します。`upload_file` に指定するバイナリはこの環境で実行可能である必要があります。

- **Linux (x86_64)**: 通常のビルドでそのまま使えます (WSL2 Ubuntu 上での動作確認済み)
- **macOS / Windows**: ネイティブビルドは動きません。Docker や WSL2 など Linux x86_64 環境でビルドするか、クロスコンパイルが必要です
- **インタプリタ言語のスクリプト (Python 等)**: 環境非依存のためそのままアップロードできます

CDK の `architecture` を `lambda.Architecture.ARM_64` に変更すれば Graviton (ARM64) で動作させることもできますが、その場合はアップロードするバイナリも ARM64 Linux 向けにビルドする必要があります。
